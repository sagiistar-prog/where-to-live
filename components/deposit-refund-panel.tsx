...
function buildEvidenceHref(result: DepositRefundResult, reportId?: string, title?: string) {
  return buildFlowHref("/evidence", {
    ...
    reportContext: compactContext([
      result.summary,
      `目标退还：${result.targetRefund.toLocaleString()} 元；谈判底线：${result.suggestedRefundFloor.toLocaleString()} 元。`,
      "扣款拆解：",
...
function buildOfficialHref(result: DepositRefundResult, reportId?: string, title?: string) {
  return buildFlowHref("/official", {
    ...
    concerns: compactContext([
      ...
      result.summary,
      `目标退还：${result.targetRefund.toLocaleString()} 元；谈判底线：${result.suggestedRefundFloor.toLocaleString()} 元；返还截止：${result.returnDeadlineText}。`,
      result.risks,
...
function buildPaymentHref(result: DepositRefundResult, reportId?: string, title?: string) {
  return buildFlowHref("/payment", {
    ...
    refundRule: `目标退还 ${result.targetRefund.toLocaleString()} 元；谈判底线 ${result.suggestedRefundFloor.toLocaleString()} 元；返还截止 ${result.returnDeadlineText}`,
    ...
    void submitPayload(
      {
        ...
        evidenceLevel: seedValue(initialInput.evidenceLevel, "部分凭据"),
        landlordReason: seedValue(
          initialInput.landlordReason,
          "已从上一步带入，需要把争议扣款、交割凭据和返还截止日拆清楚。",
        ),
      },
        ...
        <Card className="min-w-0 p-6">
          <div className="mb-6">
            ...
            <p className="mt-2 text-sm leading-6 text-muted-foreground">
              把押金、通知期、拟扣款和凭据情况放到一张表里。住哪儿会帮你拆分“明确费用”和“可争议扣款”，再给出退款目标和沟通话术。
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            ...
            <Field label="押金金额" name="depositAmount" defaultValue={seedNumber(initialInput?.depositAmount, "5200")} />
            <Field
              label="合同要求提前通知天数"
              name="requiredNoticeDays"
              defaultValue={seedNumber(initialInput?.requiredNoticeDays, "30")}
            />
            <Field
              label="通知退租日期"
              name="noticeDate"
              defaultValue={seedValue(initialInput?.noticeDate, "2026-06-03")}
              type="date"
            />
            <Field
              label="计划退租日期"
              name="moveOutDate"
              defaultValue={seedValue(initialInput?.moveOutDate, "2026-06-18")}
              type="date"
            />
            <Field
              label="合同约定返还天数"
              name="contractReturnDays"
              defaultValue={seedNumber(initialInput?.contractReturnDays, "7")}
            />
            <Field label="未结清租金" name="unpaidRent" defaultValue={seedNumber(initialInput?.unpaidRent, "0")} />
            <Field
              label="水电燃气结算"
              name="utilityBalance"
              defaultValue={seedNumber(initialInput?.utilityBalance, "260")}
            />
            <Field label="清洁费" name="cleaningFee" defaultValue={seedNumber(initialInput?.cleaningFee, "300")} />
            <Field
              label="房东主张维修扣款"
              name="damageClaim"
              defaultValue={seedNumber(initialInput?.damageClaim, "1800")}
            />
            <Field
              label="提前退租违约金"
              name="penaltyClaim"
              defaultValue={seedNumber(initialInput?.penaltyClaim, "1200")}
            />
            <div className="space-y-2 sm:col-span-2">
              <Label htmlFor="evidenceLevel">凭据完整度</Label>
              <select
                id="evidenceLevel"
                name="evidenceLevel"
                defaultValue={seedValue(initialInput?.evidenceLevel, "部分凭据")}
                className="h-11 w-full rounded-md border border-input bg-secondary px-3 text-sm outline-none focus:ring-2 focus:ring-ring"
              >
                <option>完整凭据</option>
                <option>部分凭据</option>
                <option>几乎没有凭据</option>
              </select>
            </div>
            <div className="space-y-2 sm:col-span-2">
              ...
              <Textarea
                ...
                defaultValue={seedValue(
                  initialInput?.landlordReason,
                  "房东说墙面有污渍、桌子有划痕、退租清洁不到位，要从押金里扣维修和保洁。",
                )}
              />
            </div>
          </div>
          ...
          {result ? (
            <div className="space-y-5">
              ...
              <div className="grid gap-3 sm:grid-cols-3">
                ...
                <SummaryTile icon={ShieldAlert} label="谈判底线" value={formatMoney(result.suggestedRefundFloor)} />
              </div>
                ...
                <Button asChild variant="secondary" className="h-auto justify-between gap-3 py-3">
                  <Link href={buildEvidenceHref(result, reportId, activeTitle)}>
                    <span className="flex min-w-0 items-center gap-2">
                      ...
                      <span className="truncate">同步押金凭据</span>
                    </span>
            ...
            <div className="flex min-h-[560px] flex-col justify-center rounded-md border border-border bg-secondary p-5">
              ...
              <h3 className="text-xl font-semibold">押金要从入住前开始准备</h3>
              <p className="mt-3 text-sm leading-7 text-muted-foreground">
                退租前先拆扣款、补充凭据、定返还截止日。不要在凭据不足时签署“扣款确认”或“放弃追偿”。
              </p>
            </div>
          )}
        </Card>
        ...
        <>
          <Card className="min-w-0 p-5">
            <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
              <div>
                <p className="text-sm text-muted-foreground">押金催退包</p>
                <h3 className="mt-1 text-xl font-semibold">把返还目标和争议扣款说清楚</h3>
                <p className="mt-2 text-sm leading-6 text-muted-foreground">
                  这段文字适合在交割后、返还截止日前后发给出租方或中介。重点是列明不认可的扣款、要求提供依据，并保留书面催告记录。
                </p>
              </div>
              <Button type="button" variant="outline" onClick={copyDepositMemo}>
                <Copy className="mr-2 h-4 w-4" />
                {copiedMemo ? "已复制" : "复制催退文本"}
              </Button>
            </div>
            <pre className="mt-4 whitespace-pre-wrap break-words rounded-md border border-border bg-secondary p-4 text-xs leading-6 text-muted-foreground">
          ...
          <section className="grid grid-cols-1 gap-4 lg:grid-cols-3">
            <InfoPanel title="凭据材料" icon={FileText} items={result.evidenceChecklist} />
            <InfoPanel
              title="退租时间线"
              icon={CalendarClock}
              items={result.timeline.map((item) => `${item.dateLabel}：${item.title}。${item.action}`)}
            />
            <InfoPanel title="下一步" icon={MessageSquareText} items={result.nextActions} />
          </section>
        </>
      ) : null}
    </div>
