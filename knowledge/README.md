...
在仓库根目录运行 `npm ci`、`npm run dev`，打开 `/knowledge`。默认检索 `reference-notes.json` 中 54 条带官方出处的原创简要整理，不需要模型或数据库。覆盖合同押金、公积金、备案、保租房、中介收费、退租损耗、居住安全、买房税费、贷款、纠纷处理、搬家办证及水电气账单；默认模式是摘要关键词检索。
...
当前采集 54 个来源、198,376 个清洗后字符和 735 个片段，完成真实 BGE 推理与本地索引验收；本轮没有将新语料写入生产数据库。资料覆盖全国及北上广深部分政策，不是完整法规库；`reviewed` 仅表示核对了官方来源，不代表律师审定。网页到独立向量服务的完整联调仍待验收，详见[当前验收](../docs/housing-moving-knowledge.md)。
