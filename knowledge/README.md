...
在仓库根目录运行 `npm ci`、`npm run dev`，打开 `/knowledge`。默认检索 `reference-notes.json` 中 32 条带官方出处的原创简要整理，不需要模型或数据库。覆盖合同押金、公积金、备案、保租房、中介收费、退租损耗和居住安全；默认模式是摘要关键词检索。
...
当前采集 32 个来源、146,791 个清洗后字符和 540 个片段，完成真实 BGE 推理及 pgvector 写入。资料覆盖全国及北上广深部分政策，不是完整法规库；`reviewed` 仅表示核对了官方来源，不代表律师审定。默认网页和独立向量服务分别验收，网页到独立向量服务的完整联调仍待验收，详见[扩库验收](../docs/knowledge-expansion.md)。
