...
在仓库根目录运行 `npm ci`、`npm run dev`，打开 `/knowledge`。默认检索 `reference-notes.json` 中 42 条带官方出处的原创简要整理，不需要模型或数据库。覆盖合同押金、公积金、备案、保租房、中介收费、退租损耗、居住安全、买房税费、贷款及纠纷处理；默认模式是摘要关键词检索。
...
当前采集 42 个来源、179,065 个清洗后字符和 662 个片段，完成真实 BGE 推理及 pgvector 写入。资料覆盖全国及北上广深部分政策，不是完整法规库；`reviewed` 仅表示核对了官方来源，不代表律师审定。默认网页和独立向量服务分别验收，网页到独立向量服务的完整联调仍待验收，详见[当前验收](../docs/housing-decision-knowledge.md)。
...
本次52道住房任务与独立HTTP服务的复现命令见[决策知识库验收](../docs/housing-decision-knowledge.md)。服务响应附带语料哈希，可排除误测旧索引。网页将适用条件和已知日期收在可展开区域；不需要阅读后端术语才能使用。
