...
在仓库根目录运行 `npm ci`、`npm run dev`，打开 `/knowledge`。默认检索 `reference-notes.json` 中 57 条带官方出处的原创简要整理，不需要模型或数据库。覆盖合同押金、公积金、备案、保租房、中介收费、退租损耗、居住安全、买房税费、贷款、纠纷处理、搬家办证及水电气账单；默认模式是摘要关键词检索。
...
清洗后按 320 字符切分，步长 280，保留 40 字符重叠和出处位置。`char_start`、`char_end`对应去除边缘空白后的准确正文位置；每个片段由 `chunk.schema.json` 校验。目录可用`extract_section`明确章节起止，找不到唯一边界时失败。默认不会覆盖已有索引：更新时使用新的索引文件名，验收后再切换服务。原始采集、模型缓存和向量索引均不提交 Git。
...
当前采集 57 个来源、201,905 个清洗后字符和 749 个片段，完成真实 BGE 推理与本地索引验收；本轮没有将新语料写入生产数据库。资料覆盖全国及北上广深部分政策，不是完整法规库；`reviewed` 仅表示核对了官方来源，不代表律师审定。网页到独立向量服务的完整联调仍待验收，详见[当前验收](../docs/housing-inspection-knowledge.md)。
...
后续搬家与费用扩库的67题结果见[最新记录](../docs/housing-moving-knowledge.md)。广州原通知价格表是图片，未做OCR；目录另收官方文字解读用于价格检索。不能将全文抽取成功理解成图像和附件也已入库。

看房核验扩充后共57来源、75题回归，覆盖甲醛检测服务、承重结构装修及电梯使用标志。见[最新验收](../docs/housing-inspection-knowledge.md)。
